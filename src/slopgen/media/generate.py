...
# `manual` is special: the clip is generated by the *user* in an external web tool
# (Kling/Veo/Pika) and handed back — no HF Space, so its reserve chain is empty and
# the footage stages route it through pipeline/manual.py instead of `wan_video`.
VIDEO_MODELS: dict[str, list[str]] = {
    ...
    "manual": [],  # user-assisted: clips come from the operator, not a Space
}
...
MODEL_CLIP_SECONDS: dict[str, float] = {
    ...
    "ltx-video": 4.0,
    "animatediff": 3.0,
    "manual": 6.0,  # nominal only: sizes the script; the real length is the user's clip
    "flux": 5.0,
    "turbo": 5.0,
}
...
def is_manual_model(model: str) -> bool:
    """True when clips for `model` are supplied by the user (see pipeline/manual.py)
