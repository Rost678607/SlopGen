...
# Two names are not generators at all — the material comes from the OPERATOR, so their
# reserve chains are empty and the footage stages route them through
# pipeline/manual.py instead of `wan_video`:
#   `manual` — user-assisted GENERATION: slopgen writes a prompt, the operator makes
#       the clip in an external web tool (Kling/Veo/Pika) and hands the file back.
#   `search` — user-assisted SEARCH: slopgen writes what to look for plus ready-made
#       queries, the operator finds an existing clip or photo on a stock site.
# They sit in this table so a chain stage can name either one exactly like a
# generator, and so the share/ordering machinery needs to know nothing about them.
VIDEO_MODELS: dict[str, list[str]] = {
    ...
    "manual": [],  # user-assisted generation: clips come from the operator
    "search": [],  # user-assisted search: the operator finds existing material
}
...
MODEL_CLIP_SECONDS: dict[str, float] = {
    ...
    "ltx-video": 4.0,
    "animatediff": 3.0,
    # Both operator-supplied sources are nominal only — they size the SCRIPT (how much
    # narration one beat carries) and nothing else, since the real asset is whatever
    # arrives. `manual` is the longer of the two because a hand-made Kling/Veo shot
    # typically runs 6s+, so the writer may fill it. A found clip gets trimmed to the
    # beat and a found photo has no length at all, so `search` has no such pull and
    # simply takes the house default: this is purely how long a shot sits on screen.
    "manual": 6.0,
...
def is_search_model(model: str) -> bool:
    """True when the operator FINDS the material rather than generating it."""
    return model == "search"


def is_manual_model(model: str) -> bool:
    """True when the material for `model` comes from the user (see pipeline/manual.py)
    rather than being fetched or generated automatically — either because they generate
    it (`manual`) or because they go and find it (`search`)."""
    return model in ("manual", "search")
