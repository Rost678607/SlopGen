...
WEB_RU: dict[str, str] = {
    ...
    "web.f.meta": 'сочинить заголовок, описание и теги',
    "web.f.meta.note": 'при публикации пишутся всегда — без заголовка ролик не загрузить',
    ...
    "web.f.loopahead": 'сколько тем нейронка держит про запас (0 — придумывать по ходу)',
...
WEB_EN: dict[str, str] = {
    ...
    "web.f.meta": 'write a title, a description and tags',
    "web.f.meta.note": 'a published video always gets them — it cannot be uploaded without a title',
    ...
    "web.f.loopahead": 'topics the model keeps in hand (0 = one at a time, unseen)',
...
RU.update({
    ...
    "js.loop.ahead": 'тем про запас',
    "js.loop.ahead.note": 'сколько тем нейронка держит в очереди заранее — их видно и можно править до того, как они станут роликами',
    "js.loop.thinking": 'придумывает темы',
    # the queue: every video that has not been made yet, and its own settings
    "js.q.empty": 'очередь пуста — впиши тему или попроси у нейронки',
    "js.q.blank": '(тему придумает нейронка)',
    "js.q.byai": 'тему придумала нейронка',
    "js.q.settings": 'настройки этого ролика',
    "js.q.drag": 'перетащи, чтобы переставить',
    "js.q.inherit": 'вернуть общее',
    "js.q.more": 'ещё параметры ▾',
    "js.q.less": 'свернуть параметры ▴',
    "js.q.askai": 'ещё 3 темы',
    "js.q.asked": 'темы в очереди — посмотри и поправь',
    "js.q.all": 'выделить все',
    "js.q.none": 'снять выделение',
    "js.q.picked": 'выбрано роликов:',
    "js.q.applyto": 'применить к',
    "js.q.dup": 'дублировать',
    "js.q.drop": 'удалить',
    "js.q.bulknote": 'отметь галочкой те параметры, которые меняешь — остальное у выбранных роликов останется как было',
})
EN.update({
    "js.no-good": 'no good',
    ...
    "js.loop.ahead": 'topics in hand',
    "js.loop.ahead.note": 'how many topics the model keeps waiting in the queue — read them and change them before they become videos',
    "js.loop.thinking": 'thinking of topics',
    # the queue: every video not made yet, and the settings each of them carries
    "js.q.empty": 'nothing queued — type a topic, or ask the model for some',
    "js.q.blank": '(the model picks this one)',
    "js.q.byai": 'the model thought of this one',
    "js.q.settings": 'this video’s own settings',
    "js.q.drag": 'drag to reorder',
    "js.q.inherit": 'back to the loop’s',
    "js.q.more": 'more settings ▾',
    "js.q.less": 'fewer settings ▴',
    "js.q.askai": '3 more topics',
    "js.q.asked": 'queued — have a look and change what you like',
    "js.q.all": 'pick all',
    "js.q.none": 'pick none',
    "js.q.picked": 'videos picked:',
    "js.q.applyto": 'apply to',
    "js.q.dup": 'duplicate',
    "js.q.drop": 'delete',
    "js.q.bulknote": 'tick the settings you are changing — everything else about the picked videos is left alone',
})
...
RU.update({
    ...
    "js.forget": "убрать",
    "js.demo-catalogue": "каталог движка",
    "js.demo-clones": "клонированные",
...
EN.update({
    ...
    "js.forget": "forget",
    "js.demo-catalogue": "the engine's catalogue",
    "js.demo-clones": "cloned",
...
# --------------------------------------------------------------------------
# The montage room (web/static/montage.js, pipeline/montage.py): one video's
# timeline, cut and cast by hand.

RU.update({
    "js.montage": "🎬 монтажная",
    "js.made-by-hand": "заведён вручную — стадии гоняются из монтажной",
    "web.f.byhandgo": "🎬 собрать вручную",
    "js.byhand-not-in-a-loop": "цикл так не заводится — это разовое видео, которое ты собираешь сам",
    "js.mont.press": "прогнать эту стадию сейчас",
    "js.mont.notready": "нечего ей дать — сначала то, что слева",
    "js.mont.running": "гоню стадию",
    "js.mont.watch-the-log": "что она делает, видно в логе прогона",
    "js.mont.stage-done": "прошла",
    "js.mont.empty": "видео пустое. Нажми «сценарий» наверху, чтобы его написала нейронка, или заведи строку и напиши сам",
    "js.mont.addline": "＋ строка",
    "js.mont.pipeline": "конвейер:",
    "js.mont.silentlines": "без голоса:",
    "js.mont.notrack": "кадров ещё нет. Кликни по слову внизу — с него начнётся первый кадр, — или нарежь всё сразу:",
    "js.mont.notrack-silent": "кадров ещё нет: их режут по речи, а речи пока нет. Заведи строку, напиши и озвучь.",
    "js.mont.laytrack": "нарезать по речи",
    "js.mont.nothing-written": "ничего не написано",
    "js.mont.savetext-dirty": "сохранить текст •",
    "web.mont.no": "не надо",
    "js.mont.go": "прогнать",
    "js.mont.costs-llm": "потратит вызовы нейронки",
    "js.mont.costs-nothing": "ничего готового не тронет",
    "js.mont.already-done": "эта стадия уже проходила — прогонится заново",
    "js.mont.redo.canon": "перепишет канон-справку мира",
    "js.mont.redo.lines": "ВЫБРОСИТ и напишет заново все строки, включая набранные руками:",
    "js.mont.redo.entities": "перепишет реестр вещей, записей:",
    "js.mont.redo.takes": "переозвучит те строки, у которых менялся текст; остальные возьмёт из кэша. Озвучено сейчас:",
    "js.mont.redo.cards": "снимет подобранные карточки и подберёт заново, кадров:",
    "js.mont.redo.kept": "прибитые руками кадры останутся:",
    "js.mont.redo.footage": "пересоберёт видеоряд, строк:",
    "js.mont.redo.subs": "перепишет файлы субтитров:",
    "js.mont.redo.cut": "перережет готовый ролик заново:",
    "js.mont.redo.meta": "перепишет заголовок, описание и теги:",
    "js.mont.what.canon": "собирает из лора мира сжатую справку, которую читает сценарист.",
    "js.mont.what.script": "пишет весь сценарий целиком — план и все строки.",
    "js.mont.what.entities": "собирает реестр вещей, которые повторяются в кадрах и не входят в каст.",
    "js.mont.what.tts": "озвучивает строки и снимает тайминги слов. Дорожка после этого едет.",
    "js.mont.what.picture": "режет дорожку картинки по речи и, если не «монтирую сам», подбирает карточки.",
    "js.mont.what.footage": "раскладывает выбранные карточки по сценам — то, из чего потом собирается видео.",
    "js.mont.what.subtitles": "пишет .ass с таймингами слов, который вожжётся в картинку.",
    "js.mont.what.assemble": "рендерит ролик: кадры, голос, субтитры, фильтры. Это долго.",
    "js.mont.what.metadata": "придумывает заголовок, описание и теги.",
    "web.mont.recut": "нарезать заново",
    ...
    "web.mont.hint": "клик по слову — с него начинается новый кадр; ✕ на левом краю кадра — убрать этот стык",
    ...
    "js.mont.nopicture": "— пусто —",
    "js.mont.uncut": "убрать стык: этот кадр сольётся с предыдущим",
    "js.mont.silent": "нет голоса",
    "js.mont.pickone": "выбери кадр или строку в дорожке — здесь появится всё, что с ними можно сделать",
    "js.mont.shot": "кадр",
    "js.mont.line": "строка",
    "js.mont.clearshot": "снять картинку",
    "js.mont.target": "куда смотреть",
    "js.mont.camera": "движение камеры",
    "js.mont.addkey": "＋ ключевой кадр",
    "js.mont.simple": "простое",
    "js.mont.bykeys": "по ключам",
    "js.mont.unfreeze": "оживить движения:",
    "js.mont.frozen-why": "столько кадров стоят: их движение построено, когда кроп этой карточки был запрещён целиком. Пересчитать — карточки, род движения и тайминги останутся.",
    "js.mont.unfroze": "пересчитано движений:",
    "js.mont.key-at": "на какой секунде кадра",
    "js.mont.key-close": "насколько близко",
    "js.mont.key-drop": "убрать ключ",
    "js.mont.whole": "вся картинка",
    "js.mont.keys-note": "камера идёт от ключа к ключу по прямой; до первого и после последнего стоит. Два ключа — обычный проезд, три и больше — что угодно: подержать, наехать, увести.",
    "js.mont.soft-card": "карточка мелкая — кроп её слегка мылит",
    "js.mont.camera-needs-card": "движение строится по геометрии карточки — сначала назначь картинку, ниже.",
    "js.mont.whatshown": "что показывать",
    "js.mont.playshot": "▶ проиграть кадр",
    "js.mont.move-lead": "трогается через",
    "js.mont.move-span": "едет",
    "js.mont.still": "стоит",
    "js.mont.moving": "едет",
    "js.mont.no-regions": "на карточке ничего не размечено, поэтому доступны только те движения, которым цель не нужна: держим, лёгкий наезд, медленный проезд. Размети её — появятся наезды и панорама.",
    "js.mont.newcard": "принести свою картинку для этого кадра — станет карточкой мира",
    "js.mont.addbefore": "＋ строку до",
    "js.mont.addafter": "＋ строку после",
    "js.mont.dropline": "🗑 выбросить строку",
    "js.mont.twohalves": "звук и текст правятся порознь: сохранить текст меняет только субтитры, озвучить заново — то, что слышно (и сдвигает всё, что после). Новая строка появляется немой и ничего не двигает, пока её не озвучишь.",
    "js.mont.said": "что здесь говорится",
    "js.mont.savetext": "сохранить текст",
    "js.mont.revoice": "озвучить заново",
    "js.mont.ownvoice": "прочитать самому — принеси запись, тайминги снимет распознаватель",
    "js.mont.textsaved": "текст сохранён; голос остался прежним",
    "js.mont.voicing": "озвучиваю строку…",
    "js.mont.voiced": "готово — дорожка пересобрана",
    "js.mont.aligning": "выравниваю запись по тексту…",
    "js.mont.fxsaved": "вид сохранён",
    "js.mont.nomove": "этой карточке такое движение не сделать — нечего размечено; осталось",
    "js.mont.noaudio": "дорожка голоса не открылась — играть нечего, но резать и расставлять кадры можно: линейку рисует не звук",
    "js.mont.recut-sure": "Нарезать дорожку заново по речи? Все назначенные картинки при этом слетят — стыки меняются, а карточка выбиралась под кусок.",
    "js.mont.recut-done": "нарезано заново",
    "js.mont.blocked": "Так собирать ещё рано:",
    "js.saving": "сохраняю…",
    "web.f.byhand": "монтирую сам",
    "web.byhand.note": "нейронку про картинки не спрашивают вовсе: прогон режет дорожку по речи и останавливается в монтажной, где стыки и карточки расставляешь ты. Без галочки монтажная тоже открывается — но уже поверх того, что подобрала нейронка.",
})

EN.update({
    "js.montage": "🎬 montage",
    "js.made-by-hand": "made by hand — its stages are run from the montage room",
    "web.f.byhandgo": "🎬 build it by hand",
    "js.byhand-not-in-a-loop": "a loop is not started this way — this is one video, and you are making it",
    "js.mont.press": "run this stage now",
    "js.mont.notready": "nothing to give it yet — the ones to its left first",
    "js.mont.running": "running",
    "js.mont.watch-the-log": "what it is doing shows in the run's log",
    "js.mont.stage-done": "done",
    "js.mont.empty": "this video is empty. Press `script` above to have it written, or add a line and write it yourself",
    "js.mont.addline": "＋ a line",
    "js.mont.pipeline": "pipeline:",
    "js.mont.silentlines": "no voice yet:",
    "js.mont.notrack": "no shots yet. Click a word below and one starts there — or cut the whole thing at once:",
    "js.mont.notrack-silent": "no shots yet: they are cut on the speech, and there is none. Add a line, write it and voice it.",
    "js.mont.laytrack": "cut it on the speech",
    "js.mont.nothing-written": "nothing written on it",
    "js.mont.savetext-dirty": "save the text •",
    "web.mont.no": "leave it",
    "js.mont.go": "run it",
    "js.mont.costs-llm": "spends model calls",
    "js.mont.costs-nothing": "touches nothing already made",
    "js.mont.already-done": "this stage has run before — it will run again",
    "js.mont.redo.canon": "rewrites the world's canon sheet",
    "js.mont.redo.lines": "THROWS AWAY and rewrites every line, the ones you typed included:",
    "js.mont.redo.entities": "rewrites the visual registry, entries:",
    "js.mont.redo.takes": "re-voices the lines whose text changed and reuses the cached takes for the rest. Voiced now:",
    "js.mont.redo.cards": "takes the matched cards off and matches again, shots:",
    "js.mont.redo.kept": "shots you pinned by hand are kept:",
    "js.mont.redo.footage": "lays the footage down again, lines:",
    "js.mont.redo.subs": "rewrites the subtitle files:",
    "js.mont.redo.cut": "renders the finished video again:",
    "js.mont.redo.meta": "rewrites the title, description and tags:",
    "js.mont.what.canon": "compiles the world's lore into the dense sheet the writer reads.",
    "js.mont.what.script": "writes the whole script — the plan and every line.",
    "js.mont.what.entities": "collects the things that recur across shots and are not cast.",
    "js.mont.what.tts": "voices the lines and times their words. The track moves afterwards.",
    "js.mont.what.picture": "cuts the picture track on the speech and, unless you cut it yourself, matches cards to it.",
    "js.mont.what.footage": "lays the chosen cards onto the scenes — what the video is then built out of.",
    "js.mont.what.subtitles": "writes the .ass of word timings that gets burned into the picture.",
    "js.mont.what.assemble": "renders the video: pictures, voice, subtitles, filters. This one is slow.",
    "js.mont.what.metadata": "writes the title, description and tags.",
    "web.mont.recut": "cut it again",
    ...
    "web.mont.zoom": "zoom",
    "web.mont.hint": "click a word and a shot starts there; ✕ on a shot's left edge takes that cut back",
    ...
    "js.mont.nopicture": "— empty —",
    "js.mont.uncut": "take this cut back: the shot joins the one before it",
    "js.mont.silent": "no voice",
    "js.mont.pickone": "pick a shot or a line in the track — everything you can do to it appears here",
    "js.mont.shot": "shot",
    "js.mont.line": "line",
    "js.mont.clearshot": "take the picture off",
    "js.mont.target": "where to look",
    "js.mont.camera": "what the camera does",
    "js.mont.addkey": "＋ a keyframe",
    "js.mont.simple": "simple",
    "js.mont.bykeys": "keyframes",
    "js.mont.unfreeze": "thaw the moves:",
    "js.mont.frozen-why": "this many shots do not move: their move was built while cropping that card was forbidden outright. Recompute — the cards, the kinds and the timings all stay.",
    "js.mont.unfroze": "moves recomputed:",
    "js.mont.key-at": "at which second of the shot",
    "js.mont.key-close": "how close",
    "js.mont.key-drop": "take this key out",
    "js.mont.whole": "the whole picture",
    "js.mont.keys-note": "the camera runs straight from key to key, and holds before the first and after the last. Two keys is an ordinary travel; three or more is anything — hold, come in, lead away.",
    "js.mont.soft-card": "this card is small — cropping it softens it a little",
    "js.mont.camera-needs-card": "a move is built out of the card's geometry — give the shot a picture first, below.",
    "js.mont.whatshown": "what is shown",
    "js.mont.playshot": "▶ play this shot",
    "js.mont.move-lead": "starts after",
    "js.mont.move-span": "travels for",
    "js.mont.still": "still",
    "js.mont.moving": "moving",
    "js.mont.no-regions": "nothing is marked on this card, so only the moves that need no target are available: hold, push in, drift. Mark it up and the zooms and the pan appear.",
    "js.mont.newcard": "bring your own picture for this shot — it joins the world's base",
    "js.mont.addbefore": "＋ a line before",
    "js.mont.addafter": "＋ a line after",
    "js.mont.dropline": "🗑 drop the line",
    "js.mont.twohalves": "the voice and the text are edited apart: saving the text changes only the captions, re-voicing changes what is heard (and moves everything after it). A new line arrives silent and moves nothing until you voice it.",
    "js.mont.said": "what is said here",
    "js.mont.savetext": "save the text",
    "js.mont.revoice": "say it again",
    "js.mont.ownvoice": "read it yourself — hand over a recording and the recognizer times the words",
    "js.mont.textsaved": "text saved; the voice is untouched",
    "js.mont.voicing": "voicing the line…",
    "js.mont.voiced": "done — the track is rebuilt",
    "js.mont.aligning": "timing the recording against the text…",
    "js.mont.fxsaved": "the look is saved",
    "js.mont.nomove": "this card cannot make that move — not enough is marked on it; it stays",
    "js.mont.noaudio": "the voice track would not open — there is nothing to play, but cutting and casting still work: the ruler is not made of sound",
    "js.mont.recut-sure": "Cut the track out of the speech again? Every card goes with it — the cuts move, and a card was chosen for a stretch.",
    "js.mont.recut-done": "cut again",
    "js.mont.blocked": "too early to build this:",
    "js.saving": "saving…",
