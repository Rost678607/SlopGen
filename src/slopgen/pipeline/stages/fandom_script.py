...
PIECE_RULES = (
    "\nHOW A PIECE OF THIS KIND IS BUILT — seven rules, and together they are what "
    ...
    "  • EVERY BEAT LEAVES SOMETHING OWED. Nobody is made to watch this: they are "
    "holding a phone and the next video is one movement away, so every beat has to "
    "buy the one after it. A beat buys it by answering what the last beat raised and "
    "raising something itself — a thing named and not yet explained, a rule stated "
    "whose reason is coming, a price mentioned before it is counted.\n"
    "    Two things fail that. A beat that only says what anybody would have assumed "
    "— the ordinary next step of an ordinary procedure — and a beat that completes "
    "the picture so that the rest can be guessed. Do not tell it from the beginning "
    "to the end: tell it so that each part is wanted before it arrives. The material "
    "you hold back is what makes the material you give worth hearing, and a piece "
    "that has withheld nothing by its third beat is being scrolled past during it.\n"
...
SPINE_SYSTEM = (
    ...
    '  • "open": the first line of the video, and it does TWO jobs in one breath. It '
    "NAMES THE SUBJECT — the words of the subject above are in it, plainly, so that "
    "somebody who hears only this line already knows what they are listening to. And "
    "it PROMISES something they do not know yet: the list about to be given, the thing "
    "about to go wrong, the reason it is done this strange way. Write both. An opening "
    ...
    "— what is done, by whom, with what, how often, at what price — and every fact in "
    "them comes from the records.\n"
    "    ORDER THEM BY WHAT MAKES THE NEXT ONE WANTED, NOT BY WHAT HAPPENED NEXT. "
    "Chronology is the order this material arrives in and almost always the worst one "
    "to tell it in: a thing recounted from its beginning to its end is complete "
    "before it is interesting, and a listener who can see the shape of the rest stops "
    "listening. So the strangest thing you have goes EARLY, near the promise it "
    "answers; the ordinary detail comes in WITH it, as the ground it stands on, not "
    "as its own step ahead of it. Each step should leave something the next one "
    "settles.\n"
    "    A step that is only the next ordinary thing — arriving, being registered, "
    "being handed the usual — is not a step. Either it carries something odd of its "
    "own or it goes in one clause beside something that does, and the time it would "
    "have taken is spent on the part nobody could have guessed.\n"
...
SPINE_RULE = (
    ...
    "what is coming, which is the whole job of a first beat, and it was written to be "
    "used: put it in the first beat as it stands. Reword only what saying it aloud "
    "needs, and keep BOTH halves — the naming half is the one that goes missing, and "
    "a first beat that does not say what this is about loses the listener before the "
    "second one. Naming what is happening to them is NOT the forbidden 'situating the "
    "world': that ban is on explaining where we are to somebody who has never been "
    "here.\n"
...
OPEN_RULE_FANDOM = (
    ...
    "must already know what this is about. And it PROMISES: in the same breath it "
    "leaves something they do not know yet and will stay for — the list about to be "
    "given, the thing about to go wrong, the reason it is done this strange way. "
    "Naming without promising is a heading; promising without naming is a mood; and a "
    "first beat that is merely a concrete moment — an arrival, a form being filled in "
    "— is the one that gets scrolled past, because nothing in it says why to wait. "
    "Then, in the same beat, you are inside it: no run-up (1-2 punchy sentences in "
    "all).\n"
